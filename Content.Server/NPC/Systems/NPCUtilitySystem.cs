        ...
        switch (consideration)
        {
            ...
            case TargetHealthCon con:
            {
                if (!TryComp(targetUid, out DamageableComponent? damage))
                    return 0f;
                if (!TryComp(targetUid, out MobThresholdsComponent? thresholds)) // Corvax-Forge
                    return 1f; // a bit of a hack but works
                if (con.TargetState != MobState.Invalid && _thresholdSystem.TryGetPercentageForState(targetUid, con.TargetState, damage.TotalDamage, out var percentage, thresholds)) // Corvax-Forge
